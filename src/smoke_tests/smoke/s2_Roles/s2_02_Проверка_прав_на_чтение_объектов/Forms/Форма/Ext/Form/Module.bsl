...
Функция ТекстСообщения(ПолноеИмяМетаданных, Право)
	...
	ШаблонСообщения = НСтр("ru = 'Нет роли с правом ""%1"", кроме основных ролей, на объект метаданных ""%2""'");
...
Функция ПравоДляПроверки(ПолноеИмяМетаданных)
	...
	ИначеЕсли СтрНайти(ПолноеИмяМетаданных, "Обработка.") > 0
	 Или СтрНайти(ПолноеИмяМетаданных, "Отчет.") > 0 Тогда
		Право = "Использование";
	ИначеЕсли СтрНайти(ПолноеИмяМетаданных, "Обработка.") > 0 Тогда
		Право = "Чтение";
	Иначе
		// add_plus Расширена проверка на Просмотр
		//Право = "Чтение";
		Право = "Просмотр"; 
		// add_plus
	КонецЕсли;
