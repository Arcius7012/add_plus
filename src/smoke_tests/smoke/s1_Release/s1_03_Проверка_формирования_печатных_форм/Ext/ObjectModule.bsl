...
Процедура Тест_ПолучитьПечатнуюФорму(МетаИмя, КомандаПечати, ПропускатьПустыеФормы) Экспорт // add_plus
	
	// add_plus	
	#Область УправлениеХолдингом
	
	Если МетаИмя = "ПоступлениеТоваровУслуг" 
		И КомандаПечати.Идентификатор = "АктОПринятииКУчетуОС" Тогда
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидОперации", Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ОсновныеСредства);
	КонецЕсли;
	
	Если МетаИмя = "ОтчетПроизводстваЗаСмену" 
		И КомандаПечати.Идентификатор = "МХ18" Тогда
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "МХ18");
	КонецЕсли;
	
	Если МетаИмя = "РеализацияТоваровУслуг" Тогда
		Если КомандаПечати.Идентификатор = "ТТН" Тогда
			ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидОперации", Перечисления.ВидыОперацийРеализацияТоваров.Товары);
		КонецЕсли;
		
		Если КомандаПечати.Идентификатор = "СчетЗаказ" Тогда
			ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "СчетНаОплатуПокупателю", Документы.СчетНаОплатуПокупателю.ПустаяСсылка(), ВидСравненияКомпоновкиДанных.НеРавно);
		КонецЕсли;
	КонецЕсли;
	
	Если МетаИмя = "ИзменениеУсловийАренды"
		Или МетаИмя = "ПоступлениеВАренду" Тогда
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "Проведен", Истина);	
	КонецЕсли;
	
	
	Если МетаИмя = "СчетФактураВыданный" Тогда		
		Если КомандаПечати.Идентификатор = "КорректировочнаяСправка" Тогда
			ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидСчетаФактуры", Перечисления.ВидСчетаФактурыВыставленного.КорректировочнаяСправка);
		КонецЕсли; 
		
		Если КомандаПечати.Идентификатор = "СводнаяСправка" Тогда
			ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидСчетаФактуры", Перечисления.ВидСчетаФактурыВыставленного.СводнаяСправка);
		КонецЕсли;
	...
	Для Каждого ДокСсылка Из ДокументыДляПечати Цикл
		...
		Для Каждого СтрокаТаблицы Из КоллекцияПечатныхФорм Цикл
			...
			Если ТабДок.ВысотаТаблицы = 0 И ПропускатьПустыеФормы Тогда
				КонтекстЯдра.ПропуститьТест(НСтр("ru = 'Тест пропущен: ВысотаТаблицы равна 0'"));  
			ИначеЕсли Не ПропускатьПустыеФормы Тогда
...
Процедура ДобавитьТестыДляДокумента(НаборТестов, МетаОбъект)
	...
	Для Каждого ТекущаяКомандаПечати Из КомандыПечати Цикл
		...
		ПараметрыТеста = НаборТестов.ПараметрыТеста(МетаОбъект.Имя, СтруктураКоманды, Настройки.ПропускатьПустыеФормы);
		// add_plus
...
Функция ПолучитьДокументыДляПечати(МетаОбъект, УсловияВидимости = Неопределено)
	...
	Если ТипЗнч(УсловияВидимости) = Тип("Массив") Тогда
		Для Каждого Условие Из УсловияВидимости Цикл
			...
			// Не стандартное условие
			Если Условие.Реквизит = "МХ18" Тогда
				ДопУсловия = "Док.Продукция.НомерСтроки = 1";
				Прервать;
			КонецЕсли;
