...
Процедура ТестироватьФормуНаКлиентеТестирования(ПолноеИмяФормы, ПараметрыФормы)
	...
	// add_plus Плагин поменялся, нет параметра
	...
	ПлагинТестКлиенты.ИдентифицироватьОкноПредупреждение(ТестКлиент, ПереходПоКнопкеКомандногоИнтерфейса(), ТекстыИсключенийСервер());	
	// add_plus
	ПлагинТестКлиенты.ЗакрытьВсеОткрытыеОкна(ТестКлиент); 
...
&НаСервереБезКонтекста
Функция ИмяТеста()
	// в модуле объекта для имени события используется Метаданные().Синоним, логично что бы и тут был синоним, все должно быть единообразно, а то в ЖР 2 события создаются.
	// add_plus
	ИмяСобытия = СтрШаблон("VanessaADD.Дымовые.s1_02_Проверка_компоновки_макетов_СКД");
