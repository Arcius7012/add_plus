...
Процедура ЗаполнитьНаборТестов(НаборТестов, КонтекстЯдраПараметр) Экспорт
	...
	// add_plus
	Если Не МассивТестов.Количество() Тогда 
		НаборТестов.НачатьГруппу("Пропуск теста", Ложь);
...
Функция ПроверитьРеквизитыДокументовКомментарийОтветственный(ИмяДокумента, ИмяТипаКомментарий, ИмяТипаОтветственный)
	...
	Иначе
		Если РеквизитКомментарий <> Неопределено И ЗначениеЗаполнено(ИмяТипаКомментарий) Тогда
			// add_plus Если есть определяемый тип, то смотрим его. Если нет, то срока неогр.длины
			Если ИмяТипаКомментарий <> "" Тогда
				...
				КонецЕсли;	
			Иначе
				Если РеквизитКомментарий.Тип <> ОбщегоНазначения.ОписаниеТипаСтрока(0) Тогда
					ШаблонСообщения = НСтр("ru = '%1 тип реквизита ""Комментарий"" должен быть строкой неограниченной длины'");
					ШаблонСообщения = СтроковыеУтилиты.ПодставитьПараметрыВСтроку(ШаблонСообщения, "%1", ИмяТипаКомментарий);
					ДобавитьСообщениеВРезультат(СтроковыеУтилиты, Результат, ШаблонСообщения)
				КонецЕсли;
			КонецЕсли; 
			// add_plus
	...
	Если РеквизитОтветственный = Неопределено Тогда
		...
		ДобавитьСообщениеВРезультат(СтроковыеУтилиты, Результат, ШаблонСообщения)
	Иначе
		// add_plus Если есть определяемый тип, то смотрим его. Если нет, то справочник пользователи
		Если ИмяТипаОтветственный <> "" Тогда
		...
		Иначе
			Если РеквизитОтветственный <> Неопределено И ЗначениеЗаполнено(ИмяТипаОтветственный) Тогда
				ОпределяемыйТипОтветственный = Новый ОписаниеТипов("СправочникСсылка.Пользователи");
				Если РеквизитОтветственный.Тип <> ОпределяемыйТипОтветственный Тогда
					ШаблонСообщения = НСтр("ru = '%1 тип реквизита ""Ответственный"" должен быть Справочником Пользователи'");
					ШаблонСообщения = СтроковыеУтилиты.ПодставитьПараметрыВСтроку(ШаблонСообщения, "%1", ИмяТипаОтветственный);
					ДобавитьСообщениеВРезультат(СтроковыеУтилиты, Результат, ШаблонСообщения)
				КонецЕсли;
			КонецЕсли;			
		КонецЕсли;
