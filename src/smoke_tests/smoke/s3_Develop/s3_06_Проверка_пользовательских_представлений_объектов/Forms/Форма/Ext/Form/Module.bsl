...
Процедура ТестДолжен_ПроверитьПользовательскиеПредставленияОбъектов(ПолноеИмяМетаданных) Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Не заполнены все представления объекта.'"); // add_plus
...
Функция ПроверитьПользовательскиеПредставленияОбъектовСервер(ПолноеИмяМетаданных)
	...
	// add_plus У нас надо все заполнять
	...
	ЭтоРегистр = СтрНайти(ВРег(МассивСтрок[0]), ВРег("РегистрСведений")) > 0; 
	...
	Если ЗначениеЗаполнено(ПредставлениеОбъекта)
		И ЗначениеЗаполнено(ОбъектМетаданных.ПредставлениеСписка) Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ОбъектыМетаданных(ПрефиксОбъектов, ОтборПоПрефиксу)
	...
	Для Каждого Элемент Из ОбъектыМетаданных Цикл
		Для Каждого ОбъектМетаданных Из Метаданные[Элемент.Ключ] Цикл
			...
			Если СтрНайти(ВРег(ОбъектМетаданных.Имя), "УДАЛИТЬ") Тогда
				Продолжить;
			КонецЕсли;
