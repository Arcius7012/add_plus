...
Функция ИсключенИзПроверок(ПолноеИмяМетаданных)
	...
	// add_plus
	ИслючениеВсехОбъектов = СтроковыеУтилиты.ПодставитьПараметрыВСтроку("%1.%2*", МассивСтрокИмени[0], МассивСтрокИмени[1]);
	// add_plus
	...
	// add_plus
	Если СтрНайти(ПолноеИмяМетаданных, "ТабличнаяЧасть") <> 0
		И МассивСтрокИмени.Количество() > 3 Тогда
		СтрокаПоиска = СтроковыеУтилиты.ПодставитьПараметрыВСтроку("%1.%2.ТабличнаяЧасть.%3*", МассивСтрокИмени[0], МассивСтрокИмени[1], МассивСтрокИмени[3]);
		
		Если ИсключенияИзПроверок.Получить(ВРег(СтрокаПоиска)) <> Неопределено Тогда
			Результат = Истина;	
		КонецЕсли;	
	КонецЕсли; 
	// add_plus
